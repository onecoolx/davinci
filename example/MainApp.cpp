...
void MainApp::onInit(const WebSize& windowSize)
{
    ...
    fprintf(stderr, "%s : %s : %d  size:[%d, %d]\n", __FILE__, __func__, __LINE__, windowSize.width(), windowSize.height());
}
...
{
    fprintf(stderr, "%s : %s : %d drawRect[%d, %d, %d, %d]\n", __FILE__, __func__, __LINE__, dirtyRect.x(), dirtyRect.y(), dirtyRect.width(), dirtyRect.height());
}
...
{
    fprintf(stderr, "%s : %s : %d  focus : %d \n", __FILE__, __func__, __LINE__, focus ? 1 : 0);
}
