/*
 * MediaControlsDavinci.cpp: MediaControls implements by davinci.
 *
...
#if ENABLE(VIDEO)
#include "MediaControlsDavinci.h"
...
void MediaControlAudioMuteButtonElement::defaultEventHandler(Event* event)
{
    if (event->type() == eventNames().mousedownEvent) {
        ...
        static_cast<MediaControlsDavinci*>(m_controls)->toggleVolumeSlider();
        event->setDefaultHandled();
        return;
    }
...
MediaControlsDavinci::MediaControlsDavinci(Document* document)
...
{
    return MediaControlsDavinci::createControls(document);
}

PassRefPtr<MediaControlsDavinci> MediaControlsDavinci::createControls(Document* document)
{
    if (!document->page())
        return 0;

    RefPtr<MediaControlsDavinci> controls = adoptRef(new MediaControlsDavinci(document));
...
void MediaControlsDavinci::setMediaController(MediaControllerInterface* controller)
{
...
void MediaControlsDavinci::show()
{
    if (m_isFullscreen) {
...
void MediaControlsDavinci::hide()
{
    if (m_isFullscreen) {
...
void MediaControlsDavinci::makeOpaque()
{
    if (m_isFullscreen)
        m_panel->makeOpaque();
    else
...
void MediaControlsDavinci::makeTransparent()
{
    if (m_isFullscreen)
...
void MediaControlsDavinci::reset()
{
...
void MediaControlsDavinci::bufferingProgressed()
{
...
void MediaControlsDavinci::playbackStarted()
{
    double now = m_mediaController->currentTime();
    m_playButton->updateDisplayType();
    m_fullscreenPlayButton->updateDisplayType();
...
void MediaControlsDavinci::playbackProgressed()
{
    double now = m_mediaController->currentTime();
    m_timeline->setPosition(now);
    m_fullscreenTimeline->setPosition(now);
...
void MediaControlsDavinci::playbackStopped()
{
    double now = m_mediaController->currentTime();
    m_playButton->updateDisplayType();
    m_fullscreenPlayButton->updateDisplayType();
...
void MediaControlsDavinci::updateCurrentTimeDisplay()
{
    double now = m_mediaController->currentTime();
    double duration = m_mediaController->duration();
...
void MediaControlsDavinci::reportedError()
{
    Page* page = document()->page();
    if (!page)
        return;
...
void MediaControlsDavinci::changedMute()
{
    m_muteButton->changedMute();
}

void MediaControlsDavinci::enteredFullscreen()
{
...
void MediaControlsDavinci::exitedFullscreen()
{
...
void MediaControlsDavinci::showVolumeSlider()
{
...
void MediaControlsDavinci::toggleVolumeSlider()
{
    if (!m_mediaController->hasAudio())
        return;
...
bool MediaControlsDavinci::shouldHideControls()
{
    if (m_isFullscreen)
