...
#if 0
        height = ceilf((float)m_frameView->visibleHeight()*style()->effectiveZoom());
#else
        height = ceilf((float)m_frameView->visibleHeight()); // macross change: not scale content size
#endif
    }
    return height;
}
...
int RenderView::viewWidth() const
{
    ...
    if (!printing() && m_frameView) {
#if 0
        width = ceilf((float)m_frameView->visibleWidth()*style()->effectiveZoom());
#else
        width = ceilf((float)m_frameView->visibleWidth());// macross change: not scale content size
#endif
    }
    return width;
}
