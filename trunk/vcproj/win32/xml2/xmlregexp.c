...
xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
    ...
    if (atom == NULL) {
	PERROR("add range: atom is NULL");
	return;
    }
    if (atom->type != XML_REGEXP_RANGES) {
	PERROR("add range: atom is not ranges");
	return;
    }
    if (atom->maxRanges == 0) {
	atom->maxRanges = 4;
...
xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
    ...
    if (state == NULL) {
	PERROR("add state: state is NULL");
	return;
    }
    if (target == NULL) {
	PERROR("add state: target is NULL");
	return;
    }
    /*
...
xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from,
    ...
    if (atom == NULL) {
	PERROR("genrate transition: atom == NULL");
	return(-1);
    }
...
xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
    ...
    if (CUR != '\\') {
	PERROR("Escaped sequence: expecting \\");
	return;
    }
    NEXT;
    cur = CUR;
	...
	PERROR("Wrong escape sequence, misuse of character '\\'");
    }
}
...
xmlFAParseCharRange(xmlRegParserCtxtPtr ctxt) {
	...
	PERROR("Expecting the end of a char range");
	return;
    }
    NEXTL(len);
    /* TODO check that the values are acceptable character ranges for XML */
    if (end < start) {
	PERROR("End of range is before start of range");
    } else {
        xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
		           XML_REGEXP_CHARVAL, start, end, NULL);
    }
...
xmlFAParseCharClass(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ']') {
	    ...
	    PERROR("xmlFAParseCharClass: ']' expected");
	}
    } else {
	xmlFAParseCharClassEsc(ctxt);
    }
...
xmlFAParseQuantifier(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == '}') {
	    ...
	    PERROR("Unterminated quantifier");
	}
	if (max == 0)
	    max = min;
	if (ctxt->atom != NULL) {
...
xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
	...
	if (CUR == ')') {
	    ...
	    PERROR("xmlFAParseAtom: expecting ')'");
	}
...
xmlFAParsePiece(xmlRegParserCtxtPtr ctxt) {
	...
	return(0);
    if (ctxt->atom == NULL) {
	PERROR("internal: no atom generated");
    }
...
xmlRegexpCompile(const xmlChar *regexp) {
    ...
    if (CUR != 0) {
	PERROR("xmlFAParseRegExp: extra characters");
    }
    if (ctxt->error != 0) {
