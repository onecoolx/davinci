			   ...
			   xmlNodePtr node)
{
    ...
    * An instance of a datatype that is defined as 锟絙oolean锟?    * can have the following legal literals {true, false, 1, 0}.
    */
				...
				xmlNodePtr node)
{
	...
	* the 锟絘ctual value锟?of whose ref [attribute] is the same
	* as the 锟絘ctual value锟?of its own name attribute plus
                    ...
                    xmlNodePtr node)
{
    ...
    * then the 锟絪imple ur-type definition锟?"
    */
                         ...
                         xmlNodePtr node, int topLevel)
{
    ...
    /*
    * TargetNamespace:
    * SPEC "The 锟絘ctual value锟?of the targetNamespace [attribute]
    ...
    * <restriction> among its [children] ... the 锟絘ctual value锟?of whose
    * base [attribute] must be the same as the 锟絘ctual value锟?of its own
		...
		xmlSchemaBucketPtr *bucket)
{
	...
	* namespace, regardless of the 锟絘ctual value锟?of schemaLocation, but
                     ...
                     xmlNodePtr node)
{
    ...
    */
    ...
    if (namespaceName != NULL) {
	/*
	* 1.1 If the namespace [attribute] is present, then its 锟絘ctual value锟?	* must not match the 锟絘ctual value锟?of the enclosing <schema>'s
				...
				int type)
{
    ...
    */
	...
	if (type == XML_SCHEMA_SCHEMA_INCLUDE) {
	    ...
	    * satisfied:
	    * SPEC src-include: "It is not an error for the 锟絘ctual value锟?of the
	    ...
	    * other than <annotation> then the 锟絘ctual value锟?of the
	...
	*/
	if (bucket->origTargetNamespace != NULL) {
	    ...
	    * "SII has a targetNamespace [attribute], and its 锟絘ctual
	    * value锟?is identical to the 锟絘ctual value锟?of the targetNamespace
	    * [attribute] of SII锟?(which must have such an [attribute])."
	    */
	...
	while ((IS_SCHEMA(child, "element")) ||
		...
		*/
		    ...
		    part && part->children)
		{
			...
			* some level the 锟絘ctual value锟?of whose ref
			* [attribute] is the same as the 锟絘ctual value锟?of
			...
			*/
			    ...
			    ((WXS_PARTICLE(part))->maxOccurs != 1))
			{
			    ...
			    /*
			    * SPEC src-redefine:
			    * (6.1.2) "The 锟絘ctual value锟?of both that
			    * group's minOccurs and maxOccurs [attribute]
			    * must be 1 (or 锟絘bsent锟?.
			    */
			    xmlSchemaCustomErr(ACTXT_CAST ctxt,
				XML_SCHEMAP_SRC_REDEFINE, child, NULL,
				"The redefining model group definition "
                          ...
                          xmlNodePtr node, xmlSchemaTypeType parentType)
{
    ...
    * the {content type} of the type definition 锟絩esolved锟?to by
    * the 锟絘ctual value锟?of the base [attribute]"
    */
	    ...
	    (type->flags & XML_SCHEMAS_TYPE_GLOBAL))
	{
		...
		* REDEFINE: SPEC src-redefine (5)
		* "Within the [children], each <simpleType> must have a
		* <restriction> among its [children] ... the 锟絘ctual value锟?of
		* whose base [attribute] must be the same as the 锟絘ctual value锟?		* of its own name attribute plus target namespace;"
		*/
                          ...
                          xmlNodePtr node, int topLevel)
{
	...
	* for complex content restricting the 锟絬r-type definition锟? and the
				  ...
				  xmlSchemaParserCtxtPtr ctxt)
{
	...
	/* (type definition) ... otherwise the type definition 锟絩esolved锟?	* to by the 锟絘ctual value锟?of the type [attribute] ...
	*/
	...
	*/
	...
	if (substHead == NULL) {
	    ...
	    * declaration 锟絩esolved锟?to by the 锟絘ctual value锟?	    * of the substitutionGroup [attribute], if present"
	    */
				 ...
				 xmlSchemaTypePtr type)
{
    ...
    * define the explicit members as the type definitions 锟絩esolved锟?    * to by the items in the 锟絘ctual value锟?of the memberTypes [attribute],
			    ...
			    xmlSchemaWildcardPtr curWild)
{
    ...
    */
    ...
    /*
    * 3 If both O1 and O2 are sets of (namespace names or 锟絘bsent锟?,
    ...
    * or 锟絘bsent锟?, then a pair of not and 锟絘bsent锟?must be the value.
    */
    if ((completeWild->negNsSet != NULL) &&
	...
	if (nsFound && absentFound) {
	    ...
	    * name and 锟絘bsent锟? then any must be the value.
	    */
	    completeWild->any = 1;
	    if (completeWild->nsSet != NULL) {
		xmlSchemaFreeWildcardNsSet(completeWild->nsSet);
	    ...
	    * but not 锟絘bsent锟? then a pair of not and 锟絘bsent锟?must
	    * be the value.
	    */
	    if (completeWild->nsSet != NULL) {
		xmlSchemaFreeWildcardNsSet(completeWild->nsSet);
	    ...
	    /*
	    * 5.3 If the set S includes 锟絘bsent锟?but not the negated
	    ...
	    * name or 锟絘bsent锟? then whichever of O1 or O2 is a pair of not
	...
	while (cur != NULL) {
	    if (cur->value == NULL) {
		/*
		* 6.1 If the set S includes 锟絘bsent锟? then any must be the
		* value.
		*/
		completeWild->any = 1;
		if (completeWild->nsSet != NULL) {
	...
	if (completeWild->negNsSet == NULL) {
	    /*
	    * 6.2 If the set S does not include 锟絘bsent锟? then a pair of not
	    * and 锟絘bsent锟?must be the value.
	    */
	    if (completeWild->nsSet != NULL) {
		xmlSchemaFreeWildcardNsSet(completeWild->nsSet);
		completeWild->nsSet = NULL;
			    ...
			    xmlSchemaWildcardPtr curWild)
{
    ...
    * name or 锟絘bsent锟? and the other is a set of (namespace names or
    * 锟絘bsent锟?, then that set, minus the negated value if it was in
    * the set, minus 锟絘bsent锟?if it was in the set, must be the value.
    */
	...
	*/
    ...
    /*
    * 4 If both O1 and O2 are sets of (namespace names or 锟絘bsent锟?,
    ...
    * is a negation of 锟絘bsent锟? then the one which is the negation
			  ...
			  xmlSchemaWildcardPtr super)
{
    ...
    */
	...
	return (0);
    /*
    * 2.1 sub must be a pair of not and a namespace name or 锟絘bsent锟?
    ...
    */
	...
	return (0);
    /*
    * 3.1 sub must be a set whose members are either namespace names or 锟絘bsent锟?
    */
    if (sub->nsSet != NULL) {
	/*
	...
	*/
	if (super->nsSet != NULL) {
	    ...
	    * 锟絘bsent锟?and that value must not be in sub's set.
	    */
	    cur = sub->nsSet;
	    while (cur != NULL) {
				       ...
				       xmlSchemaWildcardPtr baseWild)
{
	...
	for (i = 0; i < uses->nbItems; i++) {
			...
			WXS_ATTRUSE_DECL_TNS(bcur)))
		{
			...
			* defined in Type Derivation OK (Simple) (锟?.14.6)."
			*/
			xmlSchemaPAttrUseErr4(pctxt,
			...
			*
			* 2.1.3.1 B's 锟絜ffective value constraint锟?is
			* 锟絘bsent锟?or default.
			*/
			if ((bEffValue != NULL) &&
			    (effFixed == 1)) {
			    ...
			    /*
			    * 2.1.3.2 R's 锟絜ffective value constraint锟?is
...
not_found:
		...
		* R's {attribute declaration} must be 锟絭alid锟?with respect
	...
	*/
		...
		xmlSchemaCheckCOSNSSubset(wild, baseWild))
	{
	    ...
	    * as defined by Wildcard Subset (锟?.10.6)."
	    */
	    xmlSchemaCustomErr4(ACTXT_CAST pctxt,
	...
	/* 4.3 Unless the {base type definition} is the 锟絬r-type
	* definition锟? the complex type definition's {attribute
...
inherit_next: {}
    ...
    */
	...
	if (baseType->attributeWildcard != NULL) {
	    /*
	    * (3.2.2.1) "If the 锟絙ase wildcard锟?is non-锟絘bsent锟? then
		...
		* and {annotation} are those of the 锟絚omplete wildcard锟?
		* and whose {namespace constraint} is the intensional union
		* of the {namespace constraint} of the 锟絚omplete wildcard锟?		* and of the 锟絙ase wildcard锟? as defined in Attribute
		* Wildcard Union (锟?.10.6)."
		*/
		...
		/*
		* (3.2.2.1.1) "If the 锟絚omplete wildcard锟?is 锟絘bsent锟?
		* then the 锟絙ase wildcard锟?"
		*/
		type->attributeWildcard = baseType->attributeWildcard;
	    }
	} else {
	    /*
	    * (3.2.2.2) "otherwise (the 锟絙ase wildcard锟?is 锟絘bsent锟? the
	    * 锟絚omplete wildcard"
	    * NOOP
	    */
	}
			     ...
			     int subset)
{
    ...
    */
	...
	/*
	* 2.2.1 D's 锟絙ase type definition锟?is B.
	*/
	return (0);
    }
    /*
    * 2.2.2 D's 锟絙ase type definition锟?is not the 锟絬r-type definition锟?    * and is validly derived from B given the subset, as defined by this
    * constraint.
    */
    ...
    /*
    * 2.2.3 D's {variety} is list or union and B is the 锟絪imple ur-type
    * definition锟?
    */
			     ...
			     xmlSchemaTypePtr type)
{
    ...
    */
    /* Base type: If the datatype has been 锟絛erived锟?by 锟絩estriction锟?    * then the Simple Type Definition component from which it is 锟絛erived锟?
    * otherwise the Simple Type Definition for anySimpleType (锟?.1.6).
    */
    if (baseType == NULL) {
	/*
    ...
    */
    ...
    /*
    * 2 All simple type definitions must be derived ultimately from the 锟絪imple
    * ur-type definition (so锟?circular definitions are disallowed). That is, it
    * must be possible to reach a built-in primitive datatype or the 锟絪imple
    * ur-type definition锟?by repeatedly following the {base type definition}.
    *
			     ...
			     xmlSchemaTypePtr type)
{
	...
	*/
	...
	if (WXS_IS_ANY_SIMPLE_TYPE(type->baseType)) {
	    ...
	    * A 锟絣ist锟?datatype can be 锟絛erived锟?from an 锟絘tomic锟?datatype
	    * whose 锟絣exical space锟?allows space (such as string or anyURI)or
	    * a 锟絬nion锟?datatype any of whose {member type definitions}'s
	    * 锟絣exical space锟?allows space.
	    */
	} else {
	    /*
	    ...
	    * the empty set, as defined in Type Derivation OK (Simple) (锟?.14.6).
	    */
	    {
		...
		*/
		...
		do {
			...
			case XML_SCHEMA_FACET_WHITESPACE:
			    ...
			    * The value of 锟絯hiteSpace锟?is fixed to the value collapse.
			    */
			case XML_SCHEMA_FACET_PATTERN:
			case XML_SCHEMA_FACET_ENUMERATION:
			    break;
	...
	*/
	...
	/*
	* 3.3.1 If the {base type definition} is the 锟絪imple ur-type
	* definition锟?	*/
	if (type->baseType->builtInType == XML_SCHEMAS_ANYSIMPLETYPE) {
	    ...
	    * as defined in Type Derivation OK (Simple) (锟?.14.6).
	    */
	    {
			    ...
			    xmlSchemaTypePtr type)
{
    ...
    * Definition Schema Components (锟?.14.6).
    */
    if (WXS_IS_RESTRICTION(type)) {
	/*
				   ...
				   xmlSchemaValPtr *val)
{
	...
	* type}'s particle must be 锟絜mptiable锟?as defined by
	* Particle Emptiable (锟?.9.6)."
	*/
    ...
    * must be 锟絭alid锟?with respect to that definition as defined by String
    * Valid (锟?.14.4).
    *
    * AND
    *
    * 2.2.1 If the {content type} is a simple type definition, then the
    * string must be 锟絭alid锟?with respect to that simple type definition
    * as defined by String Valid (锟?.14.4).
    */
			     ...
			     xmlSchemaTypePtr type)
{
    ...
    * Schema Component (锟?.4.1), modulo the impact of Missing
    * Sub-components (锟?.3)."
    */
    if ((type->baseType != NULL) &&
	...
	*/
    ...
    /*
    * SPEC (3) "Circular definitions are disallowed, except for the 锟絬r-type
    * definition锟? That is, it must be possible to reach the 锟絬r-type
			     ...
			     int set)
{
    ...
    */
	...
	return (0);
    /*
    * SPEC (2.3.1) "D's {base type definition} must not be the 锟絬r-type
    * definition锟?"
    */
    if (WXS_IS_ANYTYPE(type->baseType))
	return (1);
			   ...
			   xmlSchemaTypePtr type)
{
	...
	* constraint}, as defined by Wildcard Subset (锟?.10.6)."
	*/
	...
	*/
	if ((type->contentTypeDef != NULL) &&
		...
		* complex type definition must be a 锟絭alid extension锟?		* of the {base type definition}'s particle, as defined
		* in Particle Valid (Extension) (锟?.9.6)."
		*
				      ...
				      xmlSchemaTypePtr type)
{
	...
	*/
	    ...
	    (base->contentType == XML_SCHEMA_CONTENT_BASIC))
	{
	    ...
	    * set as defined in Type Derivation OK (Simple) (锟?.14.6)."
	    *
	    ...
	    * and have a particle which is 锟絜mptiable锟?as defined in
	    * Particle Emptiable (锟?.9.6)."
	    * PASS
	    */
	} else {
	...
	*/
	if (base->contentType == XML_SCHEMA_CONTENT_EMPTY) {
	    ...
	    * which is 锟絜mptiable锟?as defined in Particle Emptiable (锟?.9.6)."
	    * PASS
	    */
	} else {
	    xmlSchemaPCustomErr(ctxt,
	...
	* must be a 锟絭alid restriction锟?of the particle of the {content
		    ...
		    xmlSchemaTypePtr type)
{
	...
	* 锟絩esolved锟?to by the 锟絘ctual value锟?of the base [attribute]
	...
	* following must be true:
	* 2.1 The type definition 锟絩esolved锟?to by the 锟絘ctual value锟?of the
    ...
    * Definition Schema Components (锟?.4.6);"
				 ...
				 xmlSchemaParticlePtr b)
{
    ...
    * occurrence range as defined by Occurrence Range OK (锟?.9.6)."
    */
			    ...
			    xmlSchemaParticlePtr b)
{
    ...
    /*
    * SPEC "For an element declaration particle to be a 锟絭alid restriction锟?    * of a wildcard particle all of the following must be true:"
    *
    * SPEC (1) "The element declaration's {target namespace} is 锟絭alid锟?    * with respect to the wildcard's {namespace constraint} as defined by
    * Wildcard allows Namespace Name (锟?.10.4)."
    */
				    ...
				    int isAnyTypeBase)
{
    ...
    * of B's {namespace constraint} as defined by Wildcard Subset (锟?.10.6)."
    */
    if (xmlSchemaCheckCOSNSSubset((xmlSchemaWildcardPtr) r->children,
	(xmlSchemaWildcardPtr) b->children))
	return (1);
    /*
    * SPEC (3) "Unless B is the content model wildcard of the 锟絬r-type
    * definition锟? R's {process contents} must be identical to or stronger
					     ...
					     xmlSchemaParticlePtr b)
{
	...
	return (-1);
    /*
    * SPEC "For a group particle to be a 锟絭alid restriction锟?of a
    * wildcard particle..."
    *
    * SPEC (1) "Every member of the {particles} of the group is a 锟絭alid
    * restriction锟?of the wildcard as defined by
    * Particle Valid (Restriction) (锟?.9.6)."
    */
			   ...
			   xmlSchemaParticlePtr b)
{
	...
	return (-1);
    /*
    * SPEC "For an all or sequence group particle to be a 锟絭alid
    * restriction锟?of another group particle with the same {compositor}..."
    *
    * SPEC (1) "R's occurrence range is a valid restriction of B's
    * occurrence range as defined by Occurrence Range OK (锟?.9.6)."
    */
					     ...
					     xmlSchemaTypePtr type)
{
    ...
    * definition in the 锟絜xplicit members锟?with the members of their
...
xmlSchemaTypeFixupWhitespace(xmlSchemaTypePtr type)
{
    ...
    */
    ...
    /*
    * For all 锟絘tomic锟?datatypes other than string (and types 锟絛erived锟?    * by 锟絩estriction锟?from it) the value of whiteSpace is fixed to
    * collapse
    */
    {
			  ...
			  xmlSchemaTypePtr type)
{
	...
	*/
	if ((particle == NULL) ||
	    ...
	    if (type->flags & XML_SCHEMAS_TYPE_MIXED) {
		/*
		* SPEC (2.1.4) "If the 锟絜ffective mixed锟?is true, then
	...
	*/
	if (WXS_IS_RESTRICTION(type)) {
	    ...
	    */
		...
		/*
		* SPEC (3.2.1)
		* "If the 锟絜ffective content锟?is empty, then the
		...
		*/
			...
			XML_SCHEMA_TYPE_ALL))
		{
		    ...
		    */
		    ...
		    /*
		    * SPEC "followed by the 锟絜ffective content锟?"
		    */
		    ...
		    const xmlChar * name ATTRIBUTE_UNUSED)
{
    ...
    */
	...
	case XML_SCHEMA_FACET_ENUMERATION: {
		...
		* It is an 锟絜rror锟?if any member of {value} is not in the
		* 锟絭alue space锟?of {base type definition}.
		*
		* minInclusive, maxInclusive, minExclusive, maxExclusive:
		* The value 锟絤ust锟?be in the
		* 锟絭alue space锟?of the 锟絙ase type锟?
		*/
		/*
				...
				xmlSchemaParserCtxtPtr ctxt)
{
    ...
    * (Schema Document) (锟?.15.3) is applied to a 锟絈Name锟?arising from
    * any <attributeGroup>s with a ref [attribute] among the [children],
    * it must not be the case that a 锟絈Name锟?is encountered at any depth
				  ...
				  xmlSchemaAttributeGroupPtr attrGr)
{
    ...
    * Group Definition Schema Component (锟?.6.1), modulo the impact of
    * Missing Sub-components (锟?.3);"
    */
			       ...
			       xmlSchemaAttributePtr attr)
{
    ...
    * Declaration Schema Component (锟?.2.1), modulo the impact of
    * Missing Sub-components (锟?.3)."
    */
	...
	* representation of its value must be 锟絭alid锟?with respect
	* to the {type definition} as defined in String Valid (锟?.14.4)."
			       ...
			       xmlSchemaElementPtr elemDecl)
{
    ...
    * Declaration Schema Component (锟?.3.1), modulo the impact of Missing
    * Sub-components (锟?.3)."
    */
	...
	/*
	* SPEC (3) "If there is a non-锟絘bsent锟?{substitution group
	...
	* affiliation}, as defined in Type Derivation OK (Complex) (锟?.4.6)
	* (if the {type definition} is complex) or as defined in
	* Type Derivation OK (Simple) (锟?.14.6) (if the {type definition} is
	* simple)."
	*
	...
	* representation of its value must be 锟絭alid锟?with respect to the
			     ...
			     xmlSchemaElementPtr elemDecl)
{
	...
	* Substitution Group OK (Transitive) (锟?.3.6)."
	*/
	...
	*/
	...
	switch (WXS_PARTICLE_TERM(cur)->type) {
		...
		if (search == 0) {
		    ...
		    * contained model group, recursively) or 锟絠mplicitly锟?		    * two or more element declaration particles with
		       ...
		       xmlSchemaValPtr y)
{
	...
	/*
	* (1) if a datatype T' is 锟絛erived锟?by 锟絩estriction锟?from an
	* atomic datatype T then the 锟絭alue space锟?of T' is a subset of
	* the 锟絭alue space锟?of T. */
	/*
	* (2) if datatypes T' and T'' are 锟絛erived锟?by 锟絩estriction锟?	* from a common atomic ancestor T then the 锟絭alue space锟絪 of T'
	* and T'' may overlap.
	*/
	if (ptx != pty)
	    return(0);
			     ...
			     xmlSchemaAttributeUsePtr use)
{
    ...
    * Component (锟?.5.1), modulo the impact of Missing
    * Sub-components (锟?.3)."
    */
				   ...
				   xmlSchemaParserCtxtPtr ctxt)
{
    ...
    * type definition 锟絩esolved锟?to by the 锟絘ctual value锟?of the type
    * [attribute], if present, otherwise the 锟絪imple ur-type definition锟?
    */
...
xmlSchemaCheckSRCRedefineFirst(xmlSchemaParserCtxtPtr pctxt)
{
	...
	*/
	...
	if (prev == NULL) {
	    ...
	    /*
	    * SPEC src-redefine:
	    * (6.2.1) "The 锟絘ctual value锟?of its own name attribute plus
	    * target namespace must successfully 锟絩esolve锟?to a model
	    * group definition in I."
	    * (7.2.1) "The 锟絘ctual value锟?of its own name attribute plus
	    * target namespace must successfully 锟絩esolve锟?to an attribute
	    * group definition in I."
...
xmlSchemaCheckSRCRedefineSecond(xmlSchemaParserCtxtPtr pctxt)
{
	...
	switch (item->type) {
		...
		* Group Definition Schema Components (锟?.7.2) must be a
		* 锟絭alid restriction锟?of the {model group} of that model
		...
		* Components (锟?.6.2) must be 锟絭alid restrictions锟?of the
		...
		* (锟?.4.6) (where references to the base type definition are
...
#define FIXHFAILURE if (pctxt->err == XML_SCHEMAP_INTERNAL) goto exit_failure;
    ...
    * Definition Schema Components (锟?.7.2) must be a 锟絭alid
    * restriction锟?of the {model group} of that model group definition
    * in I, as defined in Particle Valid (Restriction) (锟?.9.6)."
    */
	...
	if (sto->type == XPATH_STATE_OBJ_TYPE_IDC_FIELD) {
		...
		*/
		...
		if (keySeq == NULL) {
		    ...
		    * 3 For each node in the 锟絫arget node set锟?all
...
create_key:
	    ...
	    * then no two members of the 锟絨ualified node set锟?have
	    * 锟絢ey-sequences锟?whose members are pairwise equal, as
...
selector_key_error:
		...
		xmlChar *str = NULL;
		/*
		* 4.2.1 (KEY) The 锟絫arget node set锟?and the
		* 锟絨ualified node set锟?are equal, that is, every
		* member of the 锟絫arget node set锟?is also a member
		* of the 锟絨ualified node set锟?and vice versa.
		*/
		xmlSchemaCustomErr(ACTXT_CAST vctxt,
		    XML_SCHEMAV_CVC_IDC, NULL,
		    WXS_BASIC_CAST idc,
...
#define NORMALIZE(atype) \
    ...
    * the empty set, as defined in Type Derivation OK (Simple) (锟?.14.6), then
    * the string must be a 锟絛eclared entity name锟?
    */
    /*
    * 2.2 If The definition is ENTITIES or is validly derived from ENTITIES
    * given the empty set, as defined in Type Derivation OK (Simple) (锟?.14.6),
    * then every whitespace-delimited substring of the string must be a 锟絛eclared
    * entity name锟?
    */
    /*
    ...
    */
	...
	/*
	* SPEC (1.2.1) "if {variety} is 锟絘tomic锟?then the string must 锟絤atch锟?	* a literal in the 锟絣exical space锟?of {base type definition}"
	*/
	/*
	...
	*/
	...
	/* 1.2.2 if {variety} is 锟絣ist锟?then the string must be a sequence
	* of white space separated tokens, each of which 锟絤atch锟絜s a literal
	* in the 锟絣exical space锟?of {item type definition}
	*/
	/*
	...
	*/
	...
	/*
	* TODO: For all datatypes 锟絛erived锟?by 锟絬nion锟? whiteSpace does
	* not apply directly; however, the normalization behavior of 锟絬nion锟?	* types is controlled by the value of whiteSpace on that one of the
	* 锟絤emberTypes锟?against which the 锟絬nion锟?is successfully validated.
	*
	...
	*/
	...
	/*
	* 1.2.3 if {variety} is 锟絬nion锟?then the string must 锟絤atch锟?a
	* literal in the 锟絣exical space锟?of at least one member of
	...
	*/
	if ((ret == 0) && (type->flags & XML_SCHEMAS_TYPE_HAS_FACETS)) {
	    /*
	    * The normalization behavior of 锟絬nion锟?types is controlled by
	    * the value of whiteSpace on that one of the 锟絤emberTypes锟?	    * against which the 锟絬nion锟?is successfully validated.
	    */
			...
			xmlSchemaElementPtr elemDecl)
{
	...
	*localType = xmlSchemaGetType(vctxt->schema, local, nsName);
	...
	if (elemDecl != NULL) {
	    ...
	    * "The 锟絣ocal type definition锟?must be validly
...
xmlSchemaVAttributesComplex(xmlSchemaValidCtxtPtr vctxt)
{
    ...
    /*
    * SPEC (cvc-attribute)
    * (1) "The declaration must not be 锟絘bsent锟?(see Missing
    * Sub-components (锟?.3) for how this can fail to be
    * the case)."
    ...
    */
	...
	attrDecl = WXS_ATTRUSE_DECL(attrUse);
	    ...
	    * item's [namespace name] (where an 锟絘bsent锟?{target namespace}
	    * is taken to be identical to a [namespace name] with no value),
	    * then the attribute information must be 锟絭alid锟?with respect
	    * to that attribute use as per Attribute Locally Valid (Use)
	    * (锟?.5.4). In this case the {attribute declaration} of that
	    * attribute use is the 锟絚ontext-determined declaration锟?for the
	    * attribute information item with respect to Schema-Validity
	    * Assessment (Attribute) (锟?.2.4) and
	    * Assessment Outcome (Attribute) (锟?.2.5).
	    */
	...
	*/
	for (i = 0; i < nbAttrs; i++) {
	    ...
	    */
	    ...
	    /*
	    * SPEC (cvc-complex-type)
	    * (3.2.2) "The attribute information item must be 锟絭alid锟?with
	    * respect to it as defined in Item Valid (Wildcard) (锟?.10.4)."
	    *
	    * SPEC Item Valid (Wildcard) (cvc-wildcard)
	    * "... its [namespace name] must be 锟絭alid锟?with respect to
	    * the wildcard constraint, as defined in Wildcard allows
	    * Namespace Name (锟?.10.4)."
	    */
		...
		*/
		...
		if (iattr->decl != NULL) {
		    ...
		    * applied and whose 锟絭alidation锟?resulted in a
		    * 锟絚ontext-determined declaration锟?of mustFind or no
		    * 锟絚ontext-determined declaration锟?at all, and whose
		    * [local name] and [namespace name] resolve (as
		    * defined by QName resolution (Instance) (锟?.15.4)) to
			...
			*/
			...
			/*
			* SPEC (cvc-complex-type)
			* (5.2) "If 锟絯ild IDs锟?is non-empty, there must not
	...
	*/
	...
	/*
	* SPEC (cvc-attribute)
	* (3) "The item's 锟絥ormalized value锟?must be locally 锟絭alid锟?	* with respect to that {type definition} as per
	* String Valid (锟?.14.4)."
	*
	...
	*/
	...
	if (fixed) {
	    ...
	    * "For an attribute information item to be锟絭alid锟?	    * with respect to an attribute use its *normalized*
	    * value锟?must match the *canonical* lexical
	    ...
	    * (4) "The item's *actual* value锟?must match the *value* of
...
character_content:
    ...
    */
	...
	/*
	* cvc-elt (3.3.4) : 5.1.1
	* If the 锟絘ctual type definition锟?is a 锟絣ocal type definition锟?	* then the canonical lexical representation of the {value constraint}
	* value must be a valid default for the 锟絘ctual type definition锟?as
	* defined in Element Default Valid (Immediate) (锟?.3.6).
	*/
	/*
	...
	* 锟絥ormalized value锟?must be 锟絭alid锟?with respect to the
	* 锟絘ctual type definition锟?as defined by Element Locally Valid (Type)
	* (锟?.3.4).
	*/
	if (WXS_IS_SIMPLE(inode->typeDef)) {
	    ret = xmlSchemaVCheckINodeDataType(vctxt,
...
default_psvi:
	...
	*/
	...
	/*
	* 5.2.1 The element information item must be 锟絭alid锟?with respect
	* to the 锟絘ctual type definition锟?as defined by Element Locally
	* Valid (Type) (锟?.3.4).
	*/
	if (WXS_IS_SIMPLE(inode->typeDef)) {
	     /*
	    ...
	    * (Element) (锟?.3.4) did not apply, then the 锟絥ormalized value锟?	    * must be 锟絭alid锟?with respect to the type definition as defined
	    * by String Valid (锟?.14.4).
	    */
	    ret = xmlSchemaVCheckINodeDataType(vctxt,
		    inode, inode->typeDef, inode->value);
	} else if (WXS_HAS_SIMPLE_CONTENT(inode->typeDef)) {
	    ...
	    * 锟絭alid锟?with respect to the type definition as per
	    * Element Locally Valid (Complex Type) (锟?.4.4);"
	    *
	    * SPEC (cvc-complex-type) (2.2)
	    * "If the {content type} is a simple type definition, ...
	    * the 锟絥ormalized value锟?of the element information item is
	    * 锟絭alid锟?with respect to that simple type definition as
	    * defined by String Valid (锟?.14.4)."
	    */
	    ret = xmlSchemaVCheckINodeDataType(vctxt,
	...
	if ((inode->decl->value != NULL) &&
		...
		*/
		if (WXS_HAS_MIXED_CONTENT(inode->typeDef)) {
		    /*
		    * 5.2.2.2.1 If the {content type} of the 锟絘ctual type
		    * definition锟?is mixed, then the *initial value* of the
			...
			*/
		    ...
		    /*
		    * 5.2.2.2.2 If the {content type} of the 锟絘ctual type
		    * definition锟?is a simple type definition, then the
...
end_elem:
    ...
    */
    ...
    /*
    * MAYBE TODO:
    * SPEC (6) "The element information item must be 锟絭alid锟?with
    * respect to each of the {identity-constraint definitions} as per
    * Identity-constraint Satisfied (锟?.11.4)."
    */
    /*
    ...
    */
    ...
    /*
    * VAL TODO: 7 If the element information item is the 锟絭alidation root锟? it must be
    * 锟絭alid锟?per Validation Root Valid (ID/IDREF) (锟?.3.4).
    */
    return (ret);
	...
	*/
	...
	if (vctxt->inode->decl == NULL) {
	    ...
	    */
		 ...
		 *
		 * SPEC (cvc-assess-elt)
		 * "If the item cannot be 锟絪trictly assessed锟? [...]
		 * an element information item's schema validity may be laxly
		 * assessed if its 锟絚ontext-determined declaration锟?is not
		 * skip by 锟絭alidating锟?with respect to the 锟絬r-type
		 * definition锟?as per Element Locally Valid (Type) (锟?.3.4)."
		*/
		vctxt->inode->typeDef =
		    xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYTYPE);
	    }
	...
	case XML_SCHEMA_CONTENT_MIXED:
	    ...
	    * order, is 锟絭alid锟?with respect to the {content type}'s
