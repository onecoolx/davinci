...
static int contentsX(AbstractView* abstractView)
{
    ...
    if (!frameView)
        return 0;
    return frameView->contentsX() / frame->pageZoomFactor();
}
...
static int contentsY(AbstractView* abstractView)
{
    ...
    if (!frameView)
        return 0;
    return frameView->contentsY() / frame->pageZoomFactor();
}
...
void MouseRelatedEvent::computePageLocation()
{
    ...
    setAbsoluteLocation(IntPoint(roundf(pageX() * zoomFactor), roundf(pageY() * zoomFactor)));
}
    ...
    if (!isSimulated()) {
        if (RenderObject* r = targ->renderer()) {
            ...
            if (r->absolutePosition(rx, ry)) {
                int offsetX = absoluteLocation().x() - rx;
                int offsetY = absoluteLocation().y() - ry;
            	float zoomFactor = (view() && view()->frame()) ? view()->frame()->pageZoomFactor() : 1.0f;
				m_offsetX = lroundf((float)offsetX / zoomFactor);
				m_offsetY = lroundf((float)offsetY / zoomFactor);
            }
        }
    }
